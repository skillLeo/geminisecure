...
### D-071 · The one file exempt from static analysis was the one every screen depends on
Phase: 5 · Class: hardening · Found while checking a claim in the final report before writing it down
`phpstan.neon` carried a single `excludePaths` entry — `app/Http/Middleware/HandleInertiaRequests.php` — under the comment "Generated by packages, not written here, and not ours to fix." That was true when it was written. It stopped being true a long time ago: the file now shares BOTH consoles' navigation and the screen-state override with every page in the application, and everything in it past the framework's own stub is ours.
So the project's static-analysis claim was "level 6 at zero, no baseline" with an asterisk nobody was reading, and the asterisk covered the file at the centre of D-069's outage.
Two real errors were behind it: `presentUser()` returned an untyped array — a payload that ships to the browser on every page load, with no shape declared anywhere — and `$role?->label ?? …` used a nullsafe on the left of a null-coalesce, where `??` already suppresses the access. Both fixed rather than re-excluded.
Chose: remove `excludePaths` entirely. There is now no baseline and no exclusion, so the claim needs no asterisk.
...
### D-074 · Four assumptions were live in the code and had never been written into the question queue
Phase: 5 · Class: defect (process) · Found by auditing the `ASSUMPTION` markers against QUESTIONS.md
The convention this project has followed throughout: anything undecided in money, restriction, biometrics or voting sits behind a flag defaulting to the safest option, the affected code carries `// ASSUMPTION Q-0xx`, and QUESTIONS.md carries the entry a client rules on. README states it. The two halves were never checked against each other.
Grepping the markers found Q-008, Q-009, Q-010 and Q-011 live in the source — three with `ASSUMPTION` comments and one with three docblocks reading "see QUESTIONS.md Q-009" — and **none of the four in QUESTIONS.md**. Not open, not answered. The file went Q-007, then jumped to Q-012.
What they are: how far into arrears an amenity booking is blocked (restriction); who posts the cash side of an amenity deposit (money); the statutory meeting notice periods (voting); and whether a minimum tenure gates standing for the committee (voting). Every one falls inside the file's own stated admission rule, which is what makes the omission a defect rather than a judgement call — a client reading the queue to see what they had been asked would not have found them.
Worse than the gap itself: the assumptions are all reasonable and all defaulted safely. Q-010 takes the LONGER of two Jamaican readings — 21 days under the Companies Act rather than 14 under the Registration (Strata Titles) Act — and enforces by default, because publishing a meeting inside its lawful notice costs the estate the meeting and every decision taken at it. Q-011 ships OFF, because an invented tenure rule disqualifies a member from standing in their own community and no ballot can be un-closed. Good defaults nobody was told about are still decisions taken on a client's behalf.
And two of the four pointed at tests that did not exist. **Governance had no test file of any kind** — one of the four hard-stop categories, carrying two live assumptions, and nothing asserted the notice-period refusal or the tenure check. `EstateGovernanceTest` is new: 13 tests, and it caught nothing on its first run, which is the good outcome and not evidence the file was unnecessary. `EstateFacilitiesAmenitiesTest` gains the Q-009 assertion that a held and refunded deposit moves no journal line and no receivable — because "we did not get to it" and "we post nothing here deliberately" look identical in a ledger, and only one is a decision.
Chose: write all four entries in the file's own format, write the tests they name, and record this rather than quietly backfilling. A queue entry pointing at a test that does not exist is the same class of defect as an assumption with no queue entry, and I wrote one of those in the same pass before checking.
...
### D-082 · Q-002 ruled: PAYE is a band on the excess, and the rate card now says so in TAJ's own figures
Phase: 5 · Class: ruling (client) · Q-002
"The client has ruled: PAYE is 25% on the amount ABOVE the threshold. A band on the excess, never a flat rate on the whole. Your calculation was correct. The sample payslips are wrong. Unblock payroll."
What the ruling added is what the rate card did not yet hold, and each piece is load-bearing:
TWO CARDS A YEAR. The threshold changes on 1 April and income tax is assessed on a calendar year, so January to March 2026 is the 2025-04 card (1,799,376; monthly 149,948.00) and April to December the 2026-04 card (1,902,360; monthly 158,530.00). `StatutoryRateVersion::forPayDate()` is the selector, keyed on the pay date — the period end — and the card is stored on the run when it is calculated. The old code took "the latest" card; on the day this changed that would have been the 2027-04 card, not yet in force.
TAJ'S PERIODIC FIGURES STORED, NEVER DERIVED. 2026's fortnightly threshold is 73,234.90; 1,902,360 divided by 26 is 73,167.69. A calculator that divides drifts 67.21 on every fortnightly slip. The three periodic figures are columns; division survives only as the fallback for a card with none recorded — and such a card (2024-04, 2027-04, ruled on their annual figure alone) is seeded unverified, so a derived threshold can never reach an approved payslip. That reuses the approval block that already existed rather than inventing a second one.
THE 30% BAND on chargeable income above 6,000,000 a year, 500,000 a month, stored with its own rate. Where the breakpoint is measured from is Q-018.
THE NIS CEILING ROUNDED, not truncated: 416,666.67 a month as the ruling states it, where truncation gave 416,666.66 — a cent short on exactly the capped payslips.
THE PROVISIONAL CARD SUPERSEDED, NOT DELETED OR EDITED. It shares the 2026-04 effective date with the real card and pay runs point at it. Deleting it breaks their foreign key; editing its figures would silently restate every payslip computed from it. A `superseded_at` flag keeps it readable and out of the selector.
BOTH WORKED PAYSLIPS ARE GOLDEN TESTS TO THE CENT — Patricia Morgan 185,000.00 (PAYE 5,230.00, net 166,482.37, employer cost 22,930.75) and Simone Clarke 72,000.00 (PAYE nil, Education Tax 1,571.40, net 66,828.60, employer cost 8,924.40) — asserted against the SEEDED cards found by the SEEDED selector, plus the January–March variants (Patricia 7,375.50, Simone nil) that prove the selector. Simone's slip is the one that matters: nil PAYE beside non-nil Education Tax proves the threshold is a threshold, PAYE is a band, and Education Tax ignores the threshold.
PAYROLL UNBLOCKED, WITH A FIRST-LIVE-RUN ACKNOWLEDGEMENT (Part F). The figures reconcile to TAJ's tables and are not countersigned by the client's accountant, so the first run approved in each payroll — each estate's, and Gemini's own — asks the approver to tick a sentence naming the card it was reconciled against. Not a block: a tick, refused server-side without it, recorded on the run with the name and the card, and never asked again. Seeded history does not count as a live run, which is why the demonstration estate still asks on its August run.
The board's arithmetic, now identified: TAJ's 2026 FORTNIGHTLY threshold, 73,234.90, applied to monthly pay as a cliff on the whole. D-073 had reported the cliff as narrowed to a window rather than identified, and that restraint was right — 73,234.90 is not the annual figure divided by any whole number, so the natural-looking ÷26 was wrong. On the ruled figures board 15 withholds J$85,392 a month where the ruling asks J$5,230: J$80,162 a month, J$961,944 a year.
Boards 13, 15 and 16 draw the wrong PAYE and Net columns and need redrawing. They have not been touched.
Reversible: the cards are data; the ruling is not ours to reverse.
Needs client confirmation: no — this IS the ruling. Three things it sent onward are Q-016, Q-017, Q-018.

### D-083 · Employer contributions posted and remitted — D-072 unblocked
Phase: 5 · Class: ruling (client) · Q-002 second half, D-072
"Employer contributions go on the monthly S01, alongside employee deductions. S01 covers PAYE, NIS, NHT, Education Tax and HEART, and is due by the 14th of the following month. Employer Education Tax is charged on statutory income. Post: Dr Employer statutory contributions (expense) / Cr Statutory payables (liability). On remittance: Dr Statutory payables / Cr Bank."
Built: account 5010 Employer Statutory Contributions — seeded, and inserted by migration into every estate whose chart already exists, because an estate provisioned before the ruling never sees the seeder again and its first approval would otherwise fail on a missing account. Each payslip line stores the employer's NIS, NHT, Education Tax and HEART beside the employee's figures and never shows them to the employee as deductions. Approval posts one balanced entry in four lines: Dr 5000 gross, Dr 5010 employer share, Cr 2100 withheld-plus-employer, Cr 1000 net. The S01 carries both halves and filing remits `remittanceMinor()`, both halves, so 2100 returns to nil — an S01 that remitted only the employee half would leave the employer half sitting in the payable forever, looking paid on the register.
HEART is new with the ruling: 3% of gross where the employer's monthly payroll exceeds a floor. It is decided once per run on the whole payroll, not per person, and the floor defaults to zero as the ruling directed — Q-016.
Gemini's own guard payroll carries the same employer figures on its payslips and its S01, because the ruling is about what an S01 covers and Gemini is an employer too.
Seeded history posts through the same four lines, so seeded months and a live approval cannot disagree about what a paid run looks like.
Reversible: posted entries are append-only; a correction is a reversing entry.
Needs client confirmation: no — ruled.

### D-084 · A derived approver cell on Gemini payroll, and the approve route that D-021 kept out
Phase: 5 · Class: permission model + feature · Board super-admin-29
Unblocking payroll on the Gemini side needed two things the Estate side already had. The wireframe's Payroll & Accounting row carries no Approver tag, so with approval open NOBODY on the platform could approve a guard pay run. Following D-063, the tag goes to the Director — board 29's own persona and the role that already holds Full — and the Accountant keeps plain Full.
And `routes/gemini/payroll.php` deliberately had no approve route while the rates were a draft: "a route that exists but is guarded is one refactor away from being reachable". With the rates ruled, it now has exactly one write, `gemini.payroll_accounting.approve`, behind `can:gemini.payroll_accounting.approve`, and `GuardPayrollApproval` is the one door behind it.
Approve, not disburse. The board labels the control "Approve & disburse"; this platform holds no ledger for Gemini's own books and the money moves by Gemini's bank transfer, so the act records the decision, who took it and when. The run then reads Approved on board 28.
No second-person rule here, unlike the estate payroll, and deliberately: an estate run is prepared by a named person who may not approve it, while a guard run is calculated by the engine from the roster and has no preparer to exclude. The permission is the control. The first-live-run acknowledgement applies here too.
Reversible: yes.
Needs client confirmation: no — the unblocking was ruled; the cell is derived the way D-063's was.

### D-085 · Hex literals where tokens existed, and the gate that stops the next one
Phase: 5 · Class: defect (design system) + hardening · Client ruling, Part D
"Hex literals — fix, highest priority. Ten components carrying `#DCFCE7`, `#16a34a`, `#15803d` where a token exists. Your README calls an inline hex a defect; this is that, ten times. The `#0a2d52` estate gradient and the brand-mark SVG fills are correctly exempt. Add a gate that fails the build on any hex literal outside `tokens.css` and a documented allowlist — otherwise this recurs."
Fixed, every one to its token: the status pills on Gemini billing's invoice and the guard register (`--success-700` on `--success-100`), the success fills on the resident record and the vendor record (`--success-100`), the success text on the new-resident form and the payment-plan screen (`--success-700`), the SVG fills on gate activity (`--amber-700`) and the dispatch map (`--slate-500` stale, `--red-700` live), and Inertia's progress bar in `app.js` (`--navy-600`). The bar was a literal on the belief that it is read before any stylesheet parses; it is not — Inertia writes it into a style rule, and a custom property resolves when that rule applies. Laravel's stock `welcome.blade.php`, reached by no route and carrying the framework's own palette, is deleted rather than allowlisted.
`gate:tokens` scans every .vue and .js under resources/js, every stylesheet, every Blade view and every PHP file under app, and fails on a hex colour outside the allowlist. Comments are scanned deliberately: a hex in a comment is how the next person copies it — two of my own, explaining the fix by quoting the old value, tripped it on its first run and now name the token instead.
THE ALLOWLIST, each entry printed with its reason on every run: `tokens.css`, the source of truth; the lifted wireframe stylesheets and their per-board scoped copies, generated verbatim from the approved boards and never edited; the gate's own file, whose list names the literals it permits; and four single literals in single files — the estate sidebar's `#0A2D52` and the brand-mark fills in `brand.js` and the logo preview, all three exempt by the ruling, and the error page's dark gradient.
Proven both ways: it passes on the tree with sixteen allowed literals, and a probe file carrying `color: #16a34a` fails it with exit code 1.
Reversible: yes.
Needs client confirmation: one entry only — the error page's gradient (`#2D1B69` to `#0B0A1F`), which is in the approved boards and holds no token, and which the ruling did not name. Tokenising it is a design-system change, not a gate fix.

### D-086 · The deposit door, on a screen no board draws — a deliberate addition
Phase: 5 · Class: feature (off-board, deliberate) + permission model · Client ruling, Part D · Q-009
"The deposit door — build it. A service that moves money with no route is a worse gap than the caution it replaced. Put the control on the amenity booking detail, gated on `amenities · update` for hold and refund, and `amenities · approve` for forfeit — forfeit keeps a resident's money and needs the higher verb plus its required reason. Property Manager holds amenities, so this is their screen. It is not on an approved board, so log it as a deliberate addition with the reasoning, and flag the three payroll boards in the same note."
THE ADDITION. `Estate/Facilities/Booking` — one booking, the terms snapshotted onto it, and the deposit's three steps (quoted; received and held; refunded or forfeited), each read off the journal that posted it with its reference and date. Board 19's "View" link, inert until now with "not on any approved board" as its reason, opens it. It borrows board 18's shapes — the summary card, the action stack, the timeline rail — from the same batch stylesheet, so an off-board screen reads as part of its module rather than as a fourth visual language.
Why off-board is right rather than merely allowed: board 19 draws the deposit state and no control that changes it, and the ruling put the control on a detail screen that no board draws. Putting three money controls into board 19's table cells instead would have changed the board's geometry to avoid a screen the client asked for.
THE GATES, AS RULED. GET `facilities/amenities/bookings/{booking}` on `view`; POST `deposit/hold` and `deposit/refund` on `estate.facilities.update`; POST `deposit/forfeit` on `estate.facilities.approve`, with a reason required by the request and refused blank by the service. "Amenities" in the ruling is this platform's `facilities` module — there is no separate amenities permission.
NOT `accounting_posting`, which the previous version of the route test demanded. That requirement was my caution on Q-009 rather than a ruling, and it is incompatible with the ruling: the Property Manager holds Facilities and D-010 locks that role out of Accounting, so a door carrying both would be one its own persona could never open. The test that asserted no deposit route existed now pins the three routes to exactly the ruled gates and asserts the accounting gate is absent.
DERIVED APPROVER CELLS, the D-053 way. Board 24's Facilities row carries no Approver tag, so with forfeit on `approve` nobody in any estate could forfeit. The tag goes to the Community Super Admin and the Property Manager, the two roles already holding Full. The Admin Assistant keeps Entry: they may record a deposit received or returned, and may not keep one — the forfeit POST refuses them with a 403. The Treasurer holds Facilities as View and moves nothing through it. Board 24 now draws "Full · Approver" in two cells the wireframe draws as "Full".
THE HOLD TIGHTENED, because a person can now reach it. It refused only a second hold; it now also refuses a hold after the deposit was refunded or forfeited — either would post money to the bank that the estate no longer has — and a hold on a booking that was declined or cancelled.
FLAGGED IN THE SAME NOTE, AS ASKED: boards community-admin-13, 15 and 16 — pay runs, pay run approval, statutory filings — now differ from what was approved in ways nobody has seen drawn. 13 and 15 draw PAYE and Net as a cliff on the whole (D-082) while the application computes the ruled band. 15 also carries the first-live-run acknowledgement (Part F), a checkbox no board draws. 16's S01 carries the employer contributions beside the employee deductions and names HEART (D-083). All three need redrawing before a fidelity sweep can hold them to the board again; until then their residuals are content rather than layout, and should not be read as regressions.
Reversible: yes — routes and a page; the cells revert in the seeder. Entries already posted through the door are append-only and would need reversing entries.
Needs client confirmation: the two derived Approver cells (the ruling named the verb, not the roles), and the redraw of boards 13, 15 and 16.

### D-087 · The inert-control review: nine built, five opened to the roles that hold their gate, the rest tabled
Phase: 5 · Class: review (client-ordered) · Part D, Part E step 7
"The 85 inert controls — review, do not bulk-fix. Produce a table: control, screen, reason, and estimated effort to make it live. Anything under an hour that is not genuinely out of scope, build. The rest stay inert with their reason."
The table is `docs/reports/INERT_CONTROLS.md`. Eighty-five is the screen count; `gate:interactivity` counted 115 inert controls, and one more — the unit ledger's "Record manual payment" — is inert through a computed binding the gate cannot see. All 116 are in the table with a status and an effort.
WHAT WAS BUILT, AND WHY IT WAS CHEAP. Nine controls said "not built" about a screen that had since been built: board 19's View (now the booking detail, D-086), Gemini payroll's Rate table tab, the unit ledger's Place on payment plan, Send reminder now and both of its tabs (boards 7 and 8), the Notices tab on the election control room and the meetings register (board 32), and the Notices screen's own Elections tab. A stale reason is a small lie kept in reserve: each told a reader to wait for something that was already there.
FIVE OPENED BY PERMISSION, NOT BY BUILDING. The bill's work order (board 18, behind Facilities), the Vendors tab on maintenance and on bookings (board 26, behind Accounting), the dashboard's Post a notice (board 32, behind Governance create) and Gemini's Add guard (board 22, behind Guard workforce create). The target's gate is not the host screen's gate, so each is a link for a viewer holding the target's gate and an inert twin with a permission reason for everybody else — never a link that answers 403. The Vendors tab had been inert for everybody, including every role that holds Accounting; its reason said "may not hold" and was true only of the Property Manager.
ONE RULE LIFTED RATHER THAN COPIED. Which election an "Elections" tab opens — the latest the estate has held, today's year for an estate that has held none — was written inside the meetings board. The Notices tab needed the same answer, so it is `Governance::electionYear()` now and both boards read it; two copies would have been free to disagree about which election is current.
REASONS CORRECTED WHERE THE CONTROL STAYS INERT: the arrears Payment plans tab (a register of every plan is unbuilt; each household's plan is board 7), the arrears last-reminder hover (board 8 holds the log), the vendor ledger's work-order hover, and `sections.js`, whose three "not built yet" sentences described settings screens that exist and could no longer render.
NOT DONE, deliberately: nothing was bulk-fixed; Gemini payroll's Employees tab could link to Guard workforce in five minutes and was left, because it would lead out of the module — a design call; and the gate was not widened to computed bindings, because a computed `disabled` is also how a live button shows it is busy, and flagging those trains people to ignore the gate.
The gate now counts 107: the 115 less the nine built. The five opened by permission still count, because their twin is still in the markup.
Reversible: yes.
Needs client confirmation: no — ordered. The table is the deliverable, for the client to choose what is built next.

### D-088 · A seeder matched a booking by a date it computed from today, and duplicated it the next day
Phase: 5 · Class: defect (seeder) · Found re-seeding the estates after the Q-002 ruling
`Estate\FacilitiesSeeder` looked an existing booking up by `starts_at` — and computes `starts_at` from today. Re-run on the next calendar day, the lookup missed the row seeded the day before, tried to create it again and hit the unique reference: `Duplicate entry 'BKG-2026-09-0001'` on Ocean View. The seeder is meant to run over a live estate — its own deposit backfill is written so "re-seeding twice does not post the same deposit twice" — and it was re-runnable only within the day it first ran.
Fixed by matching on the booking REFERENCE, the business key the unique index already protects. A date derived from the clock is a poor identity for a row that outlives the clock reading; the reference does not move.
...
Needs client confirmation: no — ruled.
### D-090 · Payroll files kept before they leave; a document is an export at both ends; and the download route learned which module each document belongs to
Phase: 6 · Class: client-ruling (13 A2, A3) and defect (access), found while building A3
A2 — "Payroll XLSX and bank CSV are generated and not kept. Store them through the same `Documents` path as every other financial document, 7-year retention, deletion guarded." `Documents::keep()` writes the built bytes, reads them back, and marks the row READY only when what is on disk hashes to what was built; the export is released only after that. The same bytes for the same run are kept once, matched on hash, so a second export of an approved run is not a second retention clock. `content_type` is recorded, because a workbook served as a PDF is a file nobody can open.
DELETION GUARDED IN THE DATABASE. `documents_no_early_delete` refuses any delete before `retain_until`; `documents_retention_fixed` refuses a shortened date and any change to an issued document's path, size, hash or status. Triggers, not model events, for the reason the ledger and ballot triggers are: a retention promise has to hold against a query typed by hand. `gate:ledger` now requires both triggers to be installed. The fixture test truncates through the schema owner, which fires no trigger and needs a privilege the estate user lacks.
A3 — "`DocumentController::download` and `Documents::request` write no `export.taken` entry. Add it." Both now do, told apart by `stage` (`requested` / `downloaded`) with the document id, kind and hash, one row each. Every press of a request is recorded, including the one handed an existing document — the trail is of who asked. A payroll export's own entry names the kept copy.
THE DEFECT. `GET documents/{document}` checked only that the reader belonged to the estate. A Property Manager, locked out of Dues & ledger by Ruling 1, could fetch any household's statement by counting ids, and any estate user could fetch a bank file. `Document::KIND_PERMISSIONS` now opens each kind behind the gate it was asked for behind — statement on the ledger, receipt on payments, remittance on accounting, meeting papers and certificates on governance, payroll files on payroll export — and an unclassified kind opens for nobody. Tested for the Property Manager and the President.
Reversible: the column and the permission map, yes; the triggers, deliberately only by a migration.
Needs client confirmation: no — ruled, and the access fix restores what Ruling 1 already said.
### D-091 · The hardship flag suppresses a run that now exists: the automated ladder, scheduled; a lift under a minute; the flag on the arrears row
Phase: 6 · Class: client-ruling (13 A4, B2, B4)
"`automatedQueue` is never scheduled so suppression suppresses nothing in production; lifting needs no minute reference and the reason is optional; the flag does not appear on the arrears list."
THE RUN DID NOT EXIST, so scheduling the queue alone would have scheduled a list. `Collections::runAutomated()` is the run: for each owing household the queue leaves in, it skips one keeping to an agreed plan (the shield the gate and the amenity diary already give), one whose days overdue have not reached the ladder's first step, and one already sent that step or a later one in this arrears episode — the episode beginning at the oldest charge still open, so a step sent by hand counts and a second run the same morning sends nothing. Otherwise it sends the highest step reached, through the same `send()` as a deliberate notice, signed `Automated dunning run`. Notices land `queued`; no delivery adapter exists, and the log does not claim a delivery nobody made.
SKIPPING AGREED PLANS is my call, not the ruling's words: a household paying to a schedule the committee agreed should not receive a final demand for the debt the schedule is clearing. It is reported separately (`on_plan`) so it can be reversed on a word.
`dunning:run` walks live estates only — an onboarding estate's ledger is unreconciled — and one failing estate does not stop the rest. Scheduled daily at 09:00 America/Jamaica, without overlapping, on one server.
LIFT: `unit_collection_flags.lifted_minute_reference`; the service and the form both require a reason and a minute. Flags lifted before the migration keep a null — none is invented.
ARREARS ROW: `flag` (kind, label, headline) on each row with a flag in force; an amber pill under Last reminder. Nothing else on the row moves, and the flag suite still proves the board identical but for that key.
Reversible: yes.
Needs client confirmation: the plan skip only.
### D-092 · The accountant's three, ruled: HEART floor zero, a pension field at zero, the 30% band on statutory income
Phase: 6 · Class: client-ruling · 13 §0 — Q-016, Q-017, Q-018
Q-016 — "zero floor confirmed. HEART applies at 3% of gross emoluments to every employer, always." No code changed; the card's floor was already zero. The marker stays on the seeder line with the ruling written into it, so the note for the accountant lives where the figure does.
Q-017 — "None. Build the field, keep it at zero, make it configurable per employee for later." `employees.approved_pension_minor` and `guards.approved_pension_minor` (both default zero), settable on Add employee and refused if negative or above the monthly rate. `PayrollCalculator::payslip()` and `::employerCost()` take it off statutory income, so Education Tax and PAYE fall with it and NHT and NIS do not; the payslip returns and stores `pension_minor`, and net is gross less all five. Worked on Patricia Morgan at 10,000.00: net 159,207.37 against 166,482.37.
WHERE A WITHHELD PENSION POSTS. Not 2100: the S01 clears 2100, and a pension is owed to the scheme, not TAJ — crediting it there would leave it in the payable forever, looking unremitted. It credits 2150 Pension Contributions Payable. The account is NOT added to the seeded chart: with every contribution zero no estate needs it, and a chart line nobody posts to would move board 25. `Payroll::approve()` refuses a run carrying a pension until the estate has added 2150, and says so.
Q-018 — "apply 30% to statutory income above J$6,000,000 per annum / J$500,000 per month. Keep the band boundary in the rate card." `paye()` now charges 25% from the threshold to the card's `paye_higher_band_annual_minor` (per period) and 30% on statutory income above it; the marker names the alternative reading (chargeable income above 6,000,000). The 900,000 worked slip is 201,617.50, was 193,691.00. Nobody on either payroll is within 300,000 a month of the band, so no stored payslip differs.
Reversible: yes.
Needs client confirmation: no — ruled.
### D-093 · The invoice run, and the smallest set of books Gemini needs for it to post
Phase: 6 · Class: client-ruling (13 B1) and modelling
"Raise client invoice (Gemini Billing) — nothing in `app/` writes `invoices`; only the seeder does. Build the invoice run: period, unit count, per-unit price, module add-ons, subtotal, total. Posts Dr AR / Cr Service revenue."
THERE WAS NO LEDGER TO POST TO. Gemini's billing was invoice rows with a status column. Built: `platform_accounts`, `platform_journals`, `platform_journal_lines` in `gs_platform`, the estate ledger's shape and guarantees — lines first, the header insert refusing an unbalanced entry, no line added to a posted entry, lines and headers append-only by trigger and, via `grants:append-only`, by grant. The chart is two accounts because two are posted: 1100 Accounts Receivable — Clients (control, sub-ledger by client) and 4000 Subscription & Service Revenue. Not cash, not expenses: Gemini's bank and wages live in its own accounting system, and a chart pretending to be its whole books would be a second set nobody reconciles. `PlatformLedger` has one method per event (invoice raised, credit note issued), not a general post.
THE RUN. `InvoiceRun::draft()` computes the period the billing screen already projects — the month after the client's latest invoice, or the subscription's first — and its lines: the tier (units × per-unit price), each active per-guard rate × guards deployed today, each line item in force (removals and discounts as negative lines, a one-off only in the period it takes effect). Subtotal is the lines' sum; total is subtotal plus tax, which is zero — no tax is ruled (Q-019, raised). `raise()` writes invoice, lines and journal in one transaction and audits `billing.invoice_raised`. Refused: a client with no billing subscription, a period already invoiced (a correction is a credit note), a period more than a month ahead, and a total of nothing or less. Reference `{receipt prefix}-INV-{Ym}`, unique because the prefix is (D-089); seeded invoices keep their `PH-INV-` form.
CREDIT NOTES NOW POST — Dr 4000, Cr 1100 for the client — but only against an invoice that did. Invoices raised before the ledger carry no journal; none is posted retrospectively, because an opening balance invented from a status column is the figure this ledger exists to replace.
THE PREVIEW RAISES IT, behind a confirmation that states it is numbered, posted and never edited. Gated on billing `update`; the refusal reason comes from the run itself, so the control and the POST cannot disagree.
...
Reversible: yes.
Needs client confirmation: no.
### D-095 · Coverage joins the live stream, and polling becomes the fallback it always should have been
Phase: 6 · Class: client-ruling (13 C1, C2) — supersedes the "poll is never turned off" position of D-032's successors in `useLiveDispatch`
C1 — "Map, Alerts, Alertness and Alert use `useLiveDispatch`; Coverage does not. Add it." Coverage turns on clock-ins, and nothing pushed a clock-in, so subscribing alone would have refreshed the board on panics and not on the thing it reports. Built: `ShiftClocked` (`shift.clocked`) on the same private per-estate channel, broadcast now, payload `shift_id, direction, estate, is_simulated` and nothing else; `ShiftClock` announces the first clock-in and the first clock-out only, so a replayed queue pushes nothing. The coverage controller sends `estateIds`; the board re-reads KPIs and rows on a push and polls every fifteen seconds only in fallback.
BROADCAST FAILURE NEVER FAILS THE ACT. Both `ShiftClock` and `AlertIntake` now dispatch inside `rescue()`: the row is committed first, a Reverb outage is reported rather than thrown, and the handset still gets its success — the screens are in fallback polling in exactly that case. Before this, a raised panic answered the raiser's handset with a server error whenever Reverb was down, though the alert was recorded.
C2 — "Make it a fallback that engages only when the socket is down, and show the connection state in the shift bar." The poll no longer runs behind a working socket. `useAlertStream` reports connected only when the socket is connected AND the estate channel's subscription has succeeded, and clears it on any state change away from connected or a subscription error; `useLiveDispatch` stops the poll while every channel is live, starts it at the screen's full rate the moment one is lost, and refreshes once on the way back to pick up what the outage missed.
WHAT CATCHES A SILENT SOCKET NOW is the socket's own heartbeat, not a second mechanism: `pongTimeout` 10s behind the 30s `activityTimeout`, so a dead connection is declared lost within forty seconds and the screen drops to fallback from that moment. That is the honest cost of the ruling, and the bar's hover text states it.
THE SHIFT BAR. No such component existed; `DispatchConnectionBar` is it, placed beside the source badge on all five dispatch screens: "Live channel" (green dot) or "Fallback · polling every 3s · 2 min" (amber), pressable to refresh. The live map's "stale" warning now applies only in fallback — with the channel live, a quiet map is not a stale one. Fidelity re-measured: super-admin-12 1.18%, -13 1.18%, -14 1.60%, -15 1.68%, -17 1.54%.
Reversible: yes.
Needs client confirmation: no — ruled.
### D-096 · Category B defined, enumerated, flagged at the row, and enforced both ways
Phase: 6 · Class: client-ruling (13 C3)
"Define it: any screen whose primary dataset originates on a mobile device. Enumerate them in `docs/CATEGORY_B.md`, then assert in a test that every one renders `SourceBadge`. Six pages currently have it; the list will be longer."
MADE TESTABLE as: the records a screen exists to show are written by an `/api/v1` endpoint a handset calls — built today or specified in 13 D2/D3. Seventeen screens: ten Guard App (map, coverage, alerts queue, alertness, requests inbox and history, one alert, gate activity, incident log and one incident) and seven Resident App (election control room and results, maintenance queue and one ticket, booking diary and one booking, unit claims). Fourteen near misses are listed with their reasons — a dashboard that links to a badged screen, a roll-up computed from device events, an HR record, the report shell that hosts one device-derived report among seven, nominations with no app endpoint.
THE BADGE IS STILL COMPUTED FROM ROWS, so the tables had to be able to say. `is_simulated` added to `guard_requests`, `security_incidents` and, per estate, `maintenance_tickets`, `amenity_bookings`, `unit_claims` and `ballot_receipts` — on the receipt, never the mark: a receipt already names its household, and a flag on a mark is one more column to match a vote on. `SourceBadge::anySimulatedIn()` asks of the ids on the screen, never the table. The Estate Console gained the `byline` slot the Gemini console has, in the same place.
ENFORCED BOTH WAYS. `App\Support\CategoryB` is the list; `CategoryBTest` fails when a listed page lacks the badge or its required prop, when any page renders the badge without being listed, when the doc omits a screen, and when a listed server response lacks `sourceBadge` — and proves the flag moves the badge by marking one ticket simulated. Fidelity re-measured on every newly badged board: super-admin-16 0.53%, -27 0.75%; community-admin-09 0.44%, -11 0.51%, -17 1.62%, -18 1.11%, -19 0.83%, -31 0.19%.
Reversible: yes.
Needs client confirmation: the fourteen exclusions, if the client reads "primary" differently.
### D-097 · The mobile API's foundations: Ed25519 passes that verify offline, abilities from one matrix, enrolment by code, and three holes closed
Phase: 6 · Class: client-ruling (13 D1) and defect (security)
SIGNED PASSES. `visitor_passes` (per estate) holds what was signed; the QR code carries `base64url(payload).base64url(signature)` over exactly eight fields — pass_id, tenant_id, site_id, valid_from, valid_to, single_use, nonce, key_version — and nothing that names a household or a visitor, because a QR code is shown to whoever stands near it. `site_id` is the estate: today one estate is one site, and the field is there so a multi-site estate needs no new payload. `PassSigningKeys` generates an Ed25519 pair per site per version, stores the secret ENCRYPTED with the application key, decrypts it only inside `sign()`, and publishes public keys — retired versions included for 31 days, the longest a pass may run, so rotation never strands a pass already issued. `OfflinePassVerifier` is the handset's algorithm written as code with no facade, model, config or clock: signature, site, window (two minutes' tolerance) → `valid_offline`, never `valid`, because a cancellation or a use elsewhere is only known online. THE GATE: `OfflinePassVerificationTest` issues a pass, points every connection (`mysql`, `mysql_owner`, `tenant`) at a closed port, proves each `getPdo()` throws, and verifies — valid, tampered, forged, expired, not yet valid, wrong site, unknown key, malformed — with a query listener recording zero queries.
ABILITIES FROM ONE MATRIX. `App\Api\AppMatrix` is the Guard App and Resident App capability grid in the web matrix's notation; a token carries `abilitiesFor(app)` and every route's ability is computed from its catalogue entry. Not rows in `roles`: `Console` records the ruling that the apps are not consoles, and two phantom roles would draw on boards 24 and 45 where nobody can change them. The hard-coded `DeviceAbilities` and `GUARD_ABILITIES` are gone; a test fails if an ability string is written anywhere else in `app/`. The audit's missing `shifts:clock` cannot recur — the matrix gives guards `shifts:write` and the route asks for it.
ONE CATALOGUE. `App\Api\Catalogue` describes every endpoint once — method, path, apps, capability, request, response keys, errors, offline behaviour — and `routes/api.php` registers routes FROM it. Contract tests hold each response to its entry's keys (`Tests\Support\ApiContract`): a key not listed fails, which is the guard-scope allowlist, and on guard routes any money-named key or currency figure fails too.
ENROLMENT. `POST /devices/enrol` with a one-time code (`device:enrolment-code`, hashed, 24 hours, once), the handset's UID, platform and Ed25519 public key. A guard with no handset, or the same UID again, is bound at once. A different handset for a bound guard is a REBIND: 202 with a claim secret; a supervisor approves on the guard's profile (guard workforce `update`); the approval records the guard's current or next shift; the handset collects its token and only then is the old token revoked.
IDEMPOTENCY AND DEVICE TIME. `idempotent` stores each write's response against the principal and key and replays it byte for byte (`Idempotent-Replayed: true`); a different request under the same key is 422, a concurrent one 409. `device.time` records the handset's clock beside the server's on every write response with `clock_skewed` over 120 s. The first seven endpoints accept both headers without requiring them.
THREE HOLES CLOSED, found building this. (1) `tenant_id` and `guard_id` came from the request body — a guard's handset could raise an alert in another estate's name; the estate and principal now come from the token and a disagreeing body is 403 `wrong_site`. (2) Any guard token could clock any shift on the platform — now 403 `not_your_shift`. (3) Sanctum's guard list held `web`, so a signed-in console user's browser passed `auth:sanctum` with a transient token that holds every ability; `sanctum.guard` is now empty — the API answers tokens only. The simulator had been working through hole (3): it now points the Sanctum guard at each nested request and clocks each shift from its own guard's borrowed handset. Also: Sanctum's token model is pinned to the central connection, because once a request opens an estate the default connection is that estate's.
Reversible: yes, except that pre-existing handsets must be re-enrolled — their tokens carry the old ability names.
Needs client confirmation: no.
### D-098 · The Guard App's endpoints, each held to its catalogue entry — and a fourth hole: bindings ran before abilities
Phase: 6 · Class: client-ruling (13 D2) and defect (security)
"Build every one, contract-tested, with idempotency keys and device time beside server time. `GET /gate/search` returns `access_restricted` as a boolean and nothing else about money."
BUILT, forty-one guard routes in the catalogue: roster (`shifts/me/current`, `shifts/me?from&to` with worked and night hours, `preflight`, breaks, open shifts and claims), orders by version id, patrol checkpoints and scans, alertness responses, presence and the day's summary, the gate (verify, entry, exit, override, search, activity, walk-up approvals), incidents with media, duress with its cancel, requests, the guard's own payslips, messages, and `sync/batch` and `sync/pull`. New tables: `shift_breaks`, `shift_claims`, `guard_presence_pings`, `incident_media`, per estate `gate_approval_requests`; posts gain coordinates and a geofence radius; alertness checks gain issue and respond-by times.
VERIFY DOES NOT ADMIT, AND OFFLINE RECONCILES. `/gate/verify` runs the handset's own verifier and adds cancellation, use and the household's restriction; `/gate/entry` records the admission and consumes a single-use pass. Online, a cancelled, used, expired or restricted pass is refused — the guard had the facts, so admitting is an override with a reason. Offline, the handset already admitted: the entry is recorded and answered with `reconciliation` (`pass_cancelled`, `pass_already_used`, …), and `/sync/pull` hands the handset every still-unexpired pass revoked since its last pull.
THE QUEUE REPLAYS THROUGH THE REAL ENDPOINTS. `sync/batch` takes up to fifty operations, each naming its catalogue endpoint with its own Idempotency-Key and device time, and hands each to the HTTP kernel with the handset's token — so a synced write passes the same ability, validation, idempotency and invariant checks as a live one, and a retried batch replays. In order; it stops at a server failure (`not_attempted`), not at a refusal. File uploads and reads are not batchable.
LOCATION IS USED, NOT KEPT. Pre-flight and presence compare coordinates to the post and store the distance or `within_geofence`, never a coordinate; `guard_presence_pings` has no column that could hold one, and the contract test asserts it.
THE ALLOWLIST IS REAL. `GuardApiContractTest` walks one shift through every guard route, holds each response to its entry, and fails if any guard route in the catalogue went uncalled. Every write's entry documents the three clock keys `DeviceTime` adds. The money scan now matches a key segment by word — its first run flagged `allowed` for containing "owed" — and is itself tested both ways.
HOLE FOUR. Laravel ranks `SubstituteBindings` ahead of route middleware it does not know, so `{shift}` resolved before `ability:` ran: a resident's token got 404 for a shift id that did not exist and 403 for one that did, which enumerates the roster. `CheckForAnyAbility` and `ResolveDevice` now sit ahead of bindings in the priority list; a resident token gets 403 on every guard-only route whatever the id, tested across all thirty-eight.
REMOVED: `POST /standing-orders/{set}/acknowledge` with a body version. `/standing-orders/{version}/acknowledge` shares its path and signs a version by its own id, so a revision published while the screen was open cannot be signed unseen. Nothing but a test called the old form.
SCHEDULED: `alertness:run` every five minutes issues checks at 30–89 minutes per guard per hour and marks unanswered ones missed after two minutes.
Reversible: yes.
Needs client confirmation: the operating numbers chosen here — clock-in opens 60 minutes early; 11 hours' rest behind the open-shift warning; alertness every 30–89 minutes with 2 minutes to answer; a 10-second duress cancel window; 3 minutes for a household to answer a walk-up; incident detail at least 20 characters, as on the console.
### D-099 · The Resident App's endpoints — sign-in by code, the estate's approval as the only link, and a fifth hole: an idempotency hash that remembered the vote
Phase: 6 · Class: client-ruling (13 D3) and defect (ballot secrecy)
"Resident App endpoints … each contract-tested with idempotency keys and device time beside server time. elections/{id}/ballot writes ballots and receipts with no join key, unique (election_id, household_id); extend the unlinkability gate to the API path."
BUILT, forty-three routes: sign-in (`auth/otp/request`, `auth/otp/verify`, `auth/claim-unit`), `me` and the household (members, vehicles, emergency contacts, pending gate approvals), visitor passes (issue, change, cancel, share) and the resident's own e-pass, answering a guard's walk-up, panic through the shared duress endpoint, dues (invoices, balance, statement, payments, receipts, pay-intent, AutoPay), maintenance tickets with media, notices and reads, meetings and RSVPs, elections (paper, eligibility, ballot, results), amenities and bookings. New per-estate tables: `household_vehicles`, `household_emergency_contacts`, `meeting_rsvps`, `ticket_media`; `estate_settings.dues_payment_instructions`.
SIGN-IN PROVES AN ADDRESS, NOTHING MORE. A six-digit code, stored as an HMAC keyed by the application key and the destination, ten minutes, five tries, once; a second request inside a minute sends nothing new; the answer is identical whether an account exists. The account is PENDING — its token reaches `me` and the claim and is `403 account_pending` everywhere else — until the estate approves the claim on board 31. ACTIVATION IS READ FROM THE ESTATE'S RECORD: `ResolveDevice` reconciles a pending account against its claim on each request, so however an approval is made (console, import, seeder) the account follows, and nothing has to remember to tell the platform. An account is NEVER linked by matching a contact on the register: a stale email would open a household's gate to a former tenant.
SMS. No provider exists. `SMS_DRIVER=log` writes the message to the log in development; in production an empty or `log` driver reads unavailable and `sms` answers `503 sms_unavailable`. Email works.
PASSES. What is signed cannot be edited: PATCH takes the visitor's name, number, purpose and plate and refuses the window and category. A restricted household issuing a restrictable pass is refused in the gate's own words and nothing more. The e-pass is a `resident` pass — never restricted, reusable, 24 hours, reissued an hour before it ends — signed like any other, so a guard verifies it offline.
DUES — ONE HOUSEHOLD, FROM THE LEDGER. Balance and statement are the journal on 1200; an invoice's outstanding figure walks payments against the oldest charges first, the ageing's own rule. Another household's id is 404, as an id nobody has. `pay-intent` answers with the amount, the reference to quote and the estate's instructions; `autopay` refuses — dues are manual (Q-012), and a card form that talks to nothing is how a resident ends up restricted for arrears they believe they paid.
BOOKINGS CANCEL INSIDE THE BOOKING'S OWN TERMS. `Amenities::cancel` is new: pending or confirmed, not started, and outside the `cancellation_hours` snapshotted onto the booking; inside that window the office decides. No money moves — a held deposit stays held for the deposit door (D-086).
AN RSVP IS NOT ATTENDANCE. `meeting_rsvps` tells a secretary who to expect; quorum stays counted from the register taken in the room.
THE VOTE, AND HOLE FIVE. The ballot goes through `Governance::castVote` — a receipt that the household voted, marks with no column that pairs them — and the API adds eligibility, which `castVote` never checked. But the idempotency middleware stored a hash of every write's BODY beside the account that sent it, and a ballot body is three option ids: hashing every combination on the paper recovers the vote. The ballot route is now `idempotent:opaque` — its hash covers method and path only, and a different paper under the same key is answered with the first answer. `BallotUnlinkabilityTest` casts two households through the API and asserts the response names no mark, option or receipt, the two idempotency hashes are identical and equal to the path's, the stored bodies are the bare answer, the marks table is its two columns, the receipts carry no clock and share no column with the marks, and a second paper writes nothing.
"UNIQUE (ELECTION_ID, HOUSEHOLD_ID)" is `ballot_receipts (ballot_id, unit_id)`: a unit has one household (`Unit::household` is has-one), and the receipt keyed on the unit is the existing, trigger-guarded table. No second key was added.
CONTRACTED. `ResidentApiContractTest` signs in by code, claims, is approved, and walks every resident route, holding each answer to its entry and failing if any went uncalled; a guard's token is refused on all resident-only routes, and a pending account on all but its two. `ResidentSignInTest` covers enumeration, the hash, resend and lock-out.
Reversible: yes.
Needs client confirmation: an SMS provider; who writes an estate's payment instructions (the column has no console field yet); members added from the app landing pending for estate verification; a resident cancel refused inside the cancellation window; the e-pass's 24 hours.
### D-100 · MOBILE_HANDOFF.md is generated from the catalogue, and a test fails when the two disagree
Phase: 6 · Class: client-ruling (13 C4)
"Rewrite MOBILE_HANDOFF.md after Wave D: one table per endpoint (method, path, ability, request, response, every error code and shape, offline behaviour)."
`App\Api\Handoff` renders the whole document: the conventions, token flows for both apps, idempotency and device time, the offline pass algorithm step by step and the sync queue — prose written once — and, read from `Catalogue`, `AppMatrix` and the limiter's own constants, the ability grid, the error list, the rate-limit table, an index and one section per endpoint. Each section lists the endpoint's own errors and then the common errors that can actually reach it — 401 and the app refusals only where a token is taken, the idempotency and device-time refusals only on writes, a 404 wherever a path takes an id.
`php artisan api:handoff` writes it; `--check` exits 1 when stale. `MobileHandoffTest` fails when the file is not what the catalogue renders, when an entry lacks a summary or offline behaviour, or when an id-taking endpoint documents no 404.
Reversible: yes.
Needs client confirmation: no.
### D-101 · The five residuals re-confirmed from their images — two were partly ours, and a type defect ran through thirty controls
Phase: 6 · Class: client-ruling (13 E1, E2) and defect (design system)
"Confirm the five fidelity residual reasons (do not bend code). BOARD_CORRECTIONS.md per board … plus 32, 34, 10, 24 and super-admin-07 if the residual is a board error."
Re-measured, all five matched their recorded figures to the hundredth (5.52, 5.42, 3.52, 3.37, 2.30). A matching number is not a confirmed reason, so each diff image was read.
CONFIRMED AS RECORDED: super-admin-07 (three illustrative clients against two real ones, D-038), community-admin-24 (the model's 13 × 7 grid and Ruling 1's locks against the board's 10 × 6), community-admin-32 (a composer drawn mid-compose).
PARTLY OURS. Board 34's verification segment labels drew at 16px regular where the board draws 12px bold. The cause: the page's scoped `button.seg-item { font: inherit }` — written to strip the browser's button font — ties the board rule's specificity (`body.wf-… .seg-item` against `button.seg-item[data-v]`), loads after it, and so reset size and weight too. Board 10's recorded reason ("invariant text") was imprecise: the image showed fourteen position chips squeezed until their labels wrapped onto four lines and the last was clipped, because flex shrank them before the row could scroll.
FIXED, NOT BENT. The reset now names `font-family` and `line-height` only, so the board's size and weight apply; chips keep their width and the row scrolls. A scan for the same collision — a `font: inherit` reset on a class the page's board stylesheet sizes or weights — found it on thirty selectors; all thirty changed. The first scan matched classes against every board's CSS rather than the page's own and so over-matched; re-checked per page, every estate page's own sheet styles the class, and every Gemini page wears the one Gemini sheet (`wf-super-admin-01-…`, set by the console layout), which styles `text-link-sm`, `seg-item`, `req-btn`, `subnav-item` and `btn-primary-sm` alike.
MEASURED. Full sweep before and after: 77 of 82 under 2% both times. Improved: community-admin-10 3.37→2.14, -34 5.52→5.21, -32 3.52→2.80, -35 1.78→0.58, -12 1.56→0.60, -27 0.84→0.09, -14 0.90→0.57; super-admin-08 1.28→0.51, -11 1.74→1.11, -16 0.51→0.17. Worse: community-admin-36 0.64→0.74 — its row actions now draw the board's bold amber, and a "Publish" label where the board draws "View agenda" differs over more pixels. Tested by reverting that one file: 0.64 without, 0.74 with; padding and line height ruled out. Kept, because the type is the board's.
E2. `docs/reports/BOARD_CORRECTIONS.md` now carries board 24 cell by cell (the structure, the Property Manager column, the three new rows, the new column), board 34's two wrong cells, super-admin-07's client cards, bars and note, and a table of the residuals that are NOT board errors (32, 10, and board 34's mid-entry state) so nobody redraws them.
Reversible: yes.
Needs client confirmation: the board redraws, as before.